...
# ✅ Global request limit config
REQUEST_LIMIT = 5
RESET_INTERVAL = 86400  # 24 hours in seconds
request_counter = {
    "global": [0, time.time()]  # [count, last_reset_time]
}

# ✅ Key Validator: Reject wrong key as 404
@app.before_request
def block_invalid_key():
...
def handle_requests():
    ...
    count, last_reset = request_counter["global"]

    if current_time - last_reset > RESET_INTERVAL:
        request_counter["global"] = [1, current_time]
    elif count >= REQUEST_LIMIT:
        return jsonify({
            "error": "Global API limit reached. Try again after 24 hours.",
            "status": 429,
            "requests_remaining": 0
        }), 429
    else:
        request_counter["global"][0] += 1
    ...
    def process_request():
        ...
        result = {
            ...
            "UID": id,
            "status": status,
            "requests_remaining": REQUEST_LIMIT - request_counter["global"][0]
        }
        return result
